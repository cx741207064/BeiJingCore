    ...
    if ("block" === value.disp || showBtnsSet[name]) {
        ...
        if (hideBtns != null && (hideBtnsSet[name] === undefined || showBtnsSet[name])) {
            ...
            if (typeof btnTitle[name]!=="undefined") {
            	btnHtml += '<li><a id=\"' + name + '\" class="' + clas + '\" onClick=\"javascript:window.frames[0].' + func + ';\" title="'+btnTitle[name]+'">' + btnName + '</a></li>';
            }else {
            	btnHtml += '<li><a id=\"' + name + '\" class="' + clas + '\" onClick=\"javascript:window.frames[0].' + func + ';\">' + btnName + '</a></li>';
            }
...
function fileUploadMeunBtnShow() {
    ...
    if ($("#btnUpload").length > 0) {
        ...
        if (ywbm.toUpperCase() === "QYSDS_A_17ND") {
            ...
            $fszlFrame = $("#fszlFrame").attr("src", sbzsWebContextPath + "/attachmentSb/getJypfxfsUploadList.do");
        }else {

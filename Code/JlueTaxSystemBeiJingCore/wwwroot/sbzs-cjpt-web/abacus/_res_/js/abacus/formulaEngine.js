    ...
    if (typeof FormulaEngine._inited === "undefined") {
        ...
        FormulaEngine.prototype.executeWbcshFormula = function(){
            ...
            if(typeof $wbcsh !== 'undefined' && $wbcsh !== null
                ...
                $wbcsh = eval('('+$wbcsh+')');
                //遍历所有节点
                //$wbcsh = JSON.parse('{"qcs.hjbhsSyxx[0].pfkbh" : 999222,"qcs.hjbhsSyxx[0].pfkmc":"kkk"}');

                //初始化外部初始化计算公式索引，索引结构{strAssResolved(公式左边节点全路径):[id1(公式id),id2]}
                this.idxWbcshCalculate2Formula = {};
                var len = this.lstCalculateFormulas.length;
                for(var i=0;i<len;i++){
                    var formula = this.lstCalculateFormulas[i];
                    if(formula && formula.flagCompiled && formula.strAssResolved){
                        var ids = this.idxWbcshCalculate2Formula[formula.strAssResolved];
                        if(!ids){
                            ids = new Array();
                        }
                        ids.push(formula.id);
                        this.idxWbcshCalculate2Formula[formula.strAssResolved] = ids;
                    }
                }
                ...
                $.each($wbcsh, function (key, val) {
                    if(key.indexOf('[#') > -1 && Object.prototype.toString.call(val) == "[object Array]" && val.length > 0){
                        ...
                        if (key.indexOf('[') > -1) {
                            // 判断节点是否存在
                            if (!_this.existJpathInFormData("$." + key)) {
        ...
        FormulaEngine.prototype.initWbcshFormula = function(key, val, idx){
            ...
            /**
             *  从idxWbcshCalculate2Formula中取出公式左边节点全路径对于的公式id
             *  如果存在，需要把改公式对应的id赋值给外部初始化公式的id，即外部初始化公式覆盖原有公式。
             *      确保对同一个节点的赋值公式，只执行外部初始化的那条公式；
             *      但是如果有原来有多条赋值公式则外部初始化会执行多次；
             *      测试场景：
             *          1.外部初始化公式: a=123,b=456 原有公式：b=a*0.1  则执行结果为：a=123,b=456;
             *          2.外部初始化公式: a=123,b=456,c=789 原有公式：b=a*0.1,b=c*0.2  则执行结果为：a=123,b=456,c=789;
             *          3.外部初始化公式: a=123,b=456 原有公式：b=a*0.1,c=b*0.1  则执行结果为：a=123，b=456,c=45.6;
             *  如果不存在，为外部初始化公式随机生成公式id
             */
            var formulaIds = this.idxWbcshCalculate2Formula["$." + key];
            var objFormulas = new Array();
            if(formulaIds){
                var formulaIdsLen = formulaIds.length;
                for(var i=0;i<formulaIdsLen;i++){
                    objFormulas.push(init(formulaIds[i]));
                }
            }else{

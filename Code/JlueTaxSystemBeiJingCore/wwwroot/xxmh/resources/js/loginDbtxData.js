	...
	$.ajax({
        ...
        success: function(data){
        	...
        	if(typeof(res.items)!="undefined"){
        		for(var i=0;i<res.items.length&&i<showCount;i++){
    				var html = getItemsHtml(i,res.items[i]);
    				$("#znggBox").append(html);
    			}
            	if(res.items.length<=0){
    				var nullHTML='<li style=\"text-align:center;\">暂无公告</li>';
    				$("#znggBox").append(nullHTML);
    			}else if(res.items.length>6){
...
function getSstxTrHtml(num,item){
	...
	html += 	"<div class=\"\" style=\"display:none\" id=\"sstx_"+(num)+"\">"
...
//		window.parent.showBszmDetail(tzbt,$("#"+type+"_"+num).html().replace(/\n/g, "<br />"));
	//解决IE8显示样式问题
	window.parent.showBszmDetail(tzbt,$("#"+type+"_li_"+num).html());
}

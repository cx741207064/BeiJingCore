 ...
 */
var index = 0;
var checkNum=0;//是否登录请求，第一次进页面，页面请求了一次，跳转时不再进行请求
/*
...
function goPage(url,m1,m2,fromWhere,qxkzsx){
	...
	checkNum++;
	if(m1==null || m1==""){
		window.location.href="/xxmh";
		return;
	}

	var ch=$(".leftmenuul");
	if(ch.find("li").length==0){//没有子元素，可能是浏览器点了返回
		$("#curM1").val("");//情况，重新获取
	}
	...
	if(curM1!=m1 && m1!=""){//不相同，需要更新左侧菜单，并更新右侧主体部分
		...
		var getSubMenuData = window.sessionStorage.getItem(m1);
		if(getSubMenuData==null){
		//获取子菜单
		$.ajax({
	        ...
	        data:{},
	   		success:function(data){ 
   				getSubMenuCallBack(url,m1,m2,curM1,fromWhere,data);
		   		}
			});
		}else{
			getSubMenuCallBack(url,m1,m2,curM1,fromWhere,getSubMenuData);
		}
...
function getSubMenuCallBack(url,m1,m2,curM1,fromWhere,data){
				var result = eval("("+data+")");
				
				if(result.flag=="ok"){
					window.sessionStorage.setItem(m1,data);
...
function setIfrUrl(url){
	//兼容处理海关url带了中文，IE9以上打不开带问题，
	if(url.indexOf("hgjkssb")>-1){
		url=encodeURI(url);
	}
